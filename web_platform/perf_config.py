...
# 表单结构：分组 → 字段。type: str/int/float/bool/select/dict
SCHEMA = [
    {'group': '压测模式', 'icon': '⚙️', 'desc': '阶梯递增=平缓爬坡（业务逐步放量）；瞬时脉冲=短尖峰+基准并发（秒杀洪峰）',
     'fields': [
         {'section': None, 'key': 'mode', 'label': '模式', 'type': 'select', 'options': list(MODES),
          'help': 'staircase 按时间梯度爬坡；pulse 周期性瞬时峰值'},
     ]},
    {'group': '通用参数', 'icon': '🎛️', 'desc': '被测服务与并发规模',
     'fields': [
         ...
         {'section': 'common', 'key': 'run_time', 'label': '压测总时长', 'type': 'str',
          'help': '如 30s / 5m / 1h；须 ≥ 爬坡时长'},
         {'section': 'common', 'key': 'spawn_rate', 'label': '用户孵化速率(人/秒)', 'type': 'float', 'min': 0.1, 'max': 100000,
          'help': '类似 JMeter ramp-up'},
    ...
    {'group': '请求定义', 'icon': '🌐', 'desc': '被测接口的路径、方法、头与参数',
     'fields': [
    ...
    {'group': '动态参数', 'icon': '🎲', 'desc': '每次请求自动替换的随机值（如随机 userId）',
     'fields': [
         {'section': 'request.dynamic_fields.random_user_id', 'key': 'enabled', 'label': '启用随机替换', 'type': 'bool'},
         {'section': 'request.dynamic_fields.random_user_id', 'key': 'field_name', 'label': '替换字段', 'type': 'str',
          'help': '被替换的字段名，如 userId'},
         {'section': 'request.dynamic_fields.random_user_id', 'key': 'digits', 'label': '随机数字位数', 'type': 'int', 'min': 1, 'max': 32},
         {'section': 'request.dynamic_fields.random_user_id', 'key': 'as_string', 'label': '以字符串形式替换', 'type': 'bool'},
     ]},
    {'group': '阶梯递增参数', 'icon': '📶', 'desc': 'mode=staircase 时生效',
     'fields': [
         {'section': 'staircase', 'key': 'start_users', 'label': '起始并发', 'type': 'int', 'min': 0, 'max': 100000},
         {'section': 'staircase', 'key': 'step_users', 'label': '每级增加并发', 'type': 'int', 'min': 0, 'max': 100000},
         {'section': 'staircase', 'key': 'step_duration', 'label': '每级持续(秒)', 'type': 'int', 'min': 1, 'max': 3600},
         {'section': 'staircase', 'key': 'ramp_duration', 'label': '爬坡总时长', 'type': 'str',
          'help': '如 5m；到期后保持末档并发到总时长结束'},
         {'section': 'staircase', 'key': 'spawn_rate', 'label': '升档孵化速率', 'type': 'float', 'min': 0.1, 'max': 100000,
          'help': '建议 ≥ 每级增加并发 / 每级持续秒数'},
     ]},
    {'group': '脉冲峰值参数', 'icon': '⚡', 'desc': 'mode=pulse 时生效',
     'fields': [
         {'section': 'pulse', 'key': 'baseline_users', 'label': '基准并发', 'type': 'int', 'min': 0, 'max': 100000},
         {'section': 'pulse', 'key': 'peak_users', 'label': '尖峰并发', 'type': 'int', 'min': 0, 'max': 100000},
         {'section': 'pulse', 'key': 'peak_duration', 'label': '尖峰持续(秒)', 'type': 'int', 'min': 1, 'max': 3600},
         {'section': 'pulse', 'key': 'pulse_interval', 'label': '脉冲周期(秒)', 'type': 'int', 'min': 1, 'max': 36000,
          'help': '上一尖峰开始到下一尖峰开始的间隔'},
         {'section': 'pulse', 'key': 'peak_spawn_rate', 'label': '尖峰孵化速率', 'type': 'float', 'min': 0.1, 'max': 100000},
         {'section': 'pulse', 'key': 'baseline_spawn_rate', 'label': '基准孵化速率', 'type': 'float', 'min': 0.1, 'max': 100000},
     ]},
    {'group': '报告参数', 'icon': '📊', 'desc': '压测结束后自动生成的 HTML 报告',
     'fields': [
         {'section': 'report', 'key': 'output_path', 'label': '报告输出路径', 'type': 'str',
          'help': '相对 perf_test 目录，如 reports/load_test_report.html'},
         {'section': 'report', 'key': 'capture_response_body', 'label': '记录响应体样本', 'type': bool and 'bool',
