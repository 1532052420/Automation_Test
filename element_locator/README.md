...
**② 新建用例包（三件套一次生成）**
目标用例选「➕ 新建…」并填用例名后（「写入元素文件」需同步选「➕ 新建…」），两个按钮直通：

| 按钮 | 行为 |
|---|---|
| **保存并继续** | 三件套（含当前步骤）**直接入库**（同一套 py 语法校验 / 同名自动备份 / 上传人登记），弹窗关闭进入连续添加，后续步骤默认继续写入该用例 |
| **保存** | 打包为 **zip 下载**（不落库），再到平台「**用例管理 → 📦 上传用例包**」上传入库（同一套校验逻辑，适合局域网/离线上传） |
