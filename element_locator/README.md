...
**② 新建用例包（三件套一次生成）**
目标用例选「➕ 新建…」并填用例名后（「写入元素文件」需同步选「➕ 新建…」），步骤先存入**临时用例区**（内存，不写框架）：

| 按钮 | 行为 |
|---|---|
| **保存并继续** | 当前步骤存入临时用例区 → 关弹窗，自由点选元素查看；点「添加测试用例」再录下一步（用例名自动保持） |
| **保存** | 临时用例区三件套（含全部已录步骤）**打包 zip 下载** → 结束会话；再到平台「**AppUI 自动化 → 用例管理**」上传入库（同一套校验逻辑，适合局域网/离线上传） |

- 弹窗内提示「临时用例 · 已录 N 步」；临时区在内存里，刷新/关闭页面前会弹确认提醒，导出 zip 后结束
- 防误操作：「写入元素文件」与「目标用例文件」的新建状态必须**同步**——都用「➕ 新建」或都选已有文件，否则保存会被 toast 提示阻止。
...
| **🎯 ③ 登记随机弹窗** | 「添加测试用例」弹窗 → 用途③ | 把选中的关闭按钮登记进 `popupElements.py` 规则库：可配**锚点**（弹窗特征，防误关）/冷却秒数/活动页约束；保存后自动体检实查。用例执行时框架在元素等待间隙被动扫描，偶遇弹窗就截图存证（allure）并自动关闭——不为弹窗写用例。开关：`config/popup.conf`（enabled=0 停用）或 `POPUP_AUTO_CLOSE=0` |
