...
description: QA 工程师 Agent — 测试策略、测试用例设计、性能测试、安全测试
trigger:
  - /agent qa_engineer
  - /qa
  - 测试用例
...
6. **UI 图一比一验收（2026-09-21 用户强制要求）** — 任务附带 UI 图 / 可交互原型 / 设计稿时，实现完成后必须**严格按图一比一验收**：
   - 验收前先从图中**枚举元素清单**：全部字段（含下拉/输入框/按钮）、布局位置（对齐/钉右/撑满）、文案、交互；清单逐项在实现页面上核对打勾
   - 图里有的元素，实现里**一个都不能少**；字段名、按钮文案、摆放位置以图为准，不得擅自增删改
   - 验收必须覆盖**真实数据的最短用例**（如仅 1 条记录的列表），不只测数据充足时的理想布局
   - 缺一项即验收不通过，必须补齐后重新过清单，不得以「功能已实现」替代视觉与字段核对
...
// .cursorrules
{
  ...
  "rules": [
    ...
    "有 UI 图/原型时严格一比一验收：先枚举图中元素清单再逐项核对，缺一项即不通过",
  ]
}
```
